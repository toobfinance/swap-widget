...
const SwapTrades: React.FC<SwapTradesProps> = ({ trades }) => {
  ...
  return sortTrades?.length > 0 ? (
    <div
      data-open={open}
      className="flex flex-col border border-[--color-primary-border] rounded-2xl px-4 data-[open=false]:border-transparent transition-all data-[open=false]:mt-2 mt-4 data-[open=true]:pt-3"
    >
      <div
        data-open={open}
      ...
      >
        <div
          data-open={open}
          className="flex items-center justify-center w-fit border border-[--color-primary-border] px-3 rounded-full data-[open=true]:border-transparent transition-all"
        >
          <span className="text-[--color-primary-text] font-bold text-sm">Route</span>
          <div
            data-open={open}
            className="w-fit data-[open=true]:rotate-180 transition-all"
          >
            <ChevronDown className="w-3.5 h-3.5 text-[--color-primary-text]" />
          </div>
        </div>
      </button>
    </div>
